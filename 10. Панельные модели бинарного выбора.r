...
# Семинар 10. Панельные модели бинарного выбора
# --------
...
model_re <- pglm(work ~ skills +                         # формула по 
                        male +                           # аналогии с glm
                        experience +
                        I(experience ^ 2) +
                        I(skills * male) +
...
gamma_re <- gamma_re[names(gamma_re) !=               # для удобства исключим sigma_a
                           "sigma"]

# Сравним точность оценок
sd_re   <- sqrt(sigma ^ 2 + sigma_a ^ 2)
coef_df <- cbind("Real"             = gamma,
                 "Probit"           = gamma_probit,
                 "Probit Adj"       = gamma_probit * sd_re,
                 "Panel"            = gamma_re,
                 "Probit Error"     = (gamma - gamma_probit) ^ 2,
                 "Probit Adj Error" = (gamma - gamma_probit * sd_re) ^ 2,
                 "Panel Error"      = (gamma - gamma_re) ^ 2)
...
# Воспользуемся моделью Чемберлена для учета фиксированных эффектов

# Достанем все регрессоры
x <- model.matrix(model_probit)[, -1]

# Усредним регрессоры по времени
x_mean <- matrix(NA, nrow = n, ncol = ncol(x))
for (i in 1:ncol(x))
{
  x_mean[, i] <- ave(x[, i], h$id, FUN = function(x) mean(x, na.rm = TRUE)) 
}
head(x_mean, 20)

# Оценим модель, включив усредненные регрессоры
model_fe <- pglm(work ~ skills + male + experience + I(experience ^ 2) +
                                 I(skills * male) + weight + x_mean,
                          index  = "id", model  = "random", method = "bfgs",
                          R = 20, data = h, family = binomial(link = 'probit'))
summary(model_fe)

# Примечание
# Для того, чтобы снизить риски смещения оценок вследствие ошибки 
# спецфикации, можно брать и более сложные функции от усредненных
# регрессоров, например, полиномы и сплайны. Однако, такой подход
# может привести к существенному росту дисперсии оценок, поэтому
# редко используется на практике

# Сравним модели по информационным критериям
data.frame("Pooled" = AIC(model_probit),
           "RE"     = AIC(model_re),
           "FE"     = AIC(model_fe))
...
X_Boris <- model.frame(formula = formula(model_re), 
                       data = Boris)[-1]
...
li_Boris <- gamma_re[1] +                             # линейный предиктор
            sum(X_Boris * gamma_re[-1])               
prob_Boris_re <- pnorm(li_Boris,                      # вероятность
...
# используя закон больших чисел ЗБЧ (для простоты допустим, что x экзогенны):
# P(z = 1) = P(u + a > -xb) = P(u > -a - xb) = 
# E(P(u > -a - xb)) = E[E(P(u > -a - xb) | a)] =
# E[E(F(xb + a) | a)], 
...
prob_Boris_LLN <- mean(pnorm(li_Boris + a_new))          # оцениваем вероятность
  # сравним результат
data.frame("Analytical " = prob_Boris_re,
           "LLN"         = prob_Boris_LLN)
...
Boris_new <- data.frame("work" = 1,
                        ...
                        "experience" = -0.1,
                        "weight" = 0.5)
X_Boris_new <- model.frame(formula = formula(model_re), 
                           data = Boris_new)[-1]
li_Boris_new <- gamma_re[1] + sum(X_Boris * gamma_re[-1])
...
# Проверка за счет
# x0 = c(model_re$estimate[8] / sqrt(2),
#        model_re$estimate[1:7])