...
head(cbind(true = sqrt(eps.var), est = as.vector(sigma.est)))
...
lnL.R.Jac  <- jacobian(func         = HetprobitLnL,        # оцениваем асимптотическую ковариационную
                       ...
                       X            = X.mat,               # рассчитать Гессиан достаточно точным 
                       W            = W.mat,               # образом не получается
                       scale_fn     = exp,                 
                       is_aggregate = FALSE,
...
gr <- ((d[, 1] - F.est) / (F.est * (1 - F.est))) * f.est   # обобщенный остаток
...
# Переменная, необходимая для тестирования гипотез
# о параметрах нескольких моделей
h$crossind <- 1:nrow(h)

# Пробит модель, описывающая занятость
model_pr <- msel(formula = lfp ~ lnnlinc         +         # логарифм нетрудового дохода
                                age + I(age ^ 2) +         # возраст   
...
# Протестируем гипотезу с помощью LR теста
lrtest_msel(model_hpa2, model_pr)

# Сравним оценки коэффициентов
data.frame("Galland.Nychka" = coef_hpa2, 
           "Probit"         = coef_pr)

# Запрограммируем функцию для расчета предельных эффектов по возрасту
me_fn <- function(object, var = "age")
{
  # Рассчитываем предельные эффекты возраста по первой модели
  me1 <- predict(object[[1]], type = "prob", group = 1, me = var)
  
  # Рассчитываем предельные эффекты возраста по второй модели
  me2 <- predict(object[[2]], type = "prob", group = 1, me = var)
  
  # Рассчитываем разницу средних предельных эффектов
  me_diff <- mean(na.omit(me1 - me2))
}

# Протестируем гипотезу о том, что средние предельные эффекты возраста
# различаются между моделями со стандартным нормальным и альтернативным
# распределением случайной ошибки
test_me <- test_msel(list(model_pr, model_hpa2), fn = me_fn,
                     fn_args = list(var = "age"))
summary(test_me)
...
model_pr <- msel(formula  = wtp ~ mineparks  +             # открытие производства в парковых
...
# Осуществим LR-тест 
lrtest_msel(model_pr, model_hpa2)

# Сравним предсказанные вероятности
p_pr   <- predict(model_pr, type = "prob", group = 1)
p_hpa  <- predict(model_hpa, is_prob = TRUE)
p_hpa2 <- predict(model_hpa2, type = "prob", group = 1)
head(cbind(probit = p_pr, GN = p_hpa, GN2 = p_hpa2))
...
# Оценим влияние образования на вероятность занятости
...
# Добавим в данные идентифицирующую переменную
h$crossind <- 1:nrow(h)

# Подготовим формулы
work_formula  <- work ~ age + nchild + health + bachelor + master
work_formula2 <- work ~ age + nchild + health + bachelor + master | health

# Оценим пробит модель
model1 <- msel(formula = work_formula, data = h)
summary(model1)

# Оценим модель с распределением Стьюдента
# Предупреждение: может долго считаться
model2 <- msel(formula = work_formula, data = h, marginal = list(student = 1))
summary(model2)

# Оценим пробит модель с гетероскедастичностью
model3 <- msel(formula = work_formula2, data = h)
summary(model3)

# Оценим пробит модель с гетероскедастичностью и распределением Стьюдента
# Предупреждение: может долго считаться
model4 <- msel(formula = work_formula2, data = h, marginal = list(student = 1))
summary(model4)

# Сравним модели по информационным критериям
c(AIC(model1), AIC(model2), AIC(model3), AIC(model4))
c(BIC(model1), BIC(model2), BIC(model3), AIC(model4))

# Оценим средний предельный эффект магистерского уровня 
# образования в сравнении с бакалаврским
me_fn <- function(object)
{
  # P(Work = 1 | X, basic = 0, bachelor = 0, master = 1)
  p1 <- predict(object, type = "prob", group = 1,
                exogenous = list(basic  = 0, bachelor  = 0, master  = 1,
                                 sbasic = 0, sbachelor = 0, smaster = 1))
  
  # P(Work = 1 | X, basic = 0, bachelor = 1, master = 0)
  p0 <- predict(object, type = "prob", group = 1,
                exogenous = list(basic  = 0, bachelor  = 1, master  = 0,
                                 sbasic = 0, sbachelor = 1, smaster = 0))
  
  # Средний предельный эффект
  me <- mean(na.omit(p1 - p0))
  
  # Возвращаем результат
  return(me)
}

# Протестируем гипотезу о значимости среднего предельного
# эффекта в одной из моделей
me_test <- test_msel(model3, fn = me_fn)
summary(me_test)

# Рассмотрим разницу средних предельных эффектов магистерского 
# уровня образования в сравнении с бакалаврским между моделями
me2_fn <- function(object)
{
  # Средний предельный эффект, оцененный по первой модели
  me1 <- me_fn(object[[1]])
  
  # Средний предельный эффект, оцененный по второй модели
  me2 <- me_fn(object[[2]])
  
  # Разница оценок средних предельных эффектов
  me_diff <- me1 - me2
...
# Сравним предельные эффекты между двумя моделями
me2_test <- test_msel(list(model1, model3), fn = me2_fn)
summary(me2_test)

# Оценим аналогичную модель для супруга
swork_formula  <- swork ~ sage + snchild + shealth + sbachelor + smaster
model1s        <- msel(formula = swork_formula, data = h)
summary(model1s)

# Оценим средний предельный эффект магистерского
# уровня образования по сравнению с бакалаврским
me3_test <- test_msel(model1s, fn = me_fn)
summary(me3_test)

# Сравним предельные эффекты между моделями для женщин и их супругов
me4_test <- test_msel(list(model1, model1s), fn = me2_fn)
summary(me4_test)
