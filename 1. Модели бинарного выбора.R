...
threshold <- 0.5                                           # порог отсечения
  # пробит модель
probs.probit   <- predict(model.probit, newdata = h.test,
                          type = "response")               # вероятность
def.probit     <- as.numeric(probs.probit >= threshold)    # дефолт
correct.probit <- mean(def.probit == h.test$def)           # доля верных прогнозов
  # логит модель
probs.logit    <- predict(model.logit, newdata = h.test, 
                          type = "response")               # вероятность
...
acc <- function(x_true, x_pred)
{
  ...
  df <- data.frame(coccrect    = mean(pred_true, na.rm = TRUE),  # доля верных прогнозов
...
#---------------------------------------------------
# Часть 9. Самостоятельное оценивание пробит модели
#---------------------------------------------------
