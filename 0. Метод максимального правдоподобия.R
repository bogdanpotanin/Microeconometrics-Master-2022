...
{                                         
  ...
  sigma <- par[2]                          # значения оптимизируемых параметров
...
lnL(par = c(mu, sigma), X = X)             # с использованием истинных значений
                                           # параметров распределения
lnL(par = c(10, 10), X = X)                # в произвольной точке
lnL(par = c(5, 8), X = X,                  # в произвольной точке и вернем
...
sigma_MLE_0 <- sqrt(var(X) * (n - 1) / n)  # по умолчанию  функция var() считает 
                                           ...
                                           # sigma совпадает с квадратным корнем из
                                           # не скорректированой
...
# Статистика в случае линейных гипотез: 
# W = (Rx-q)' * (Rx * As.Cov(x) * Rx') ^ (-1) * (Rx - q), где
