...
#----------------------------------------------------------
# Часть 1. Получение реализаций оценок метода максимального 
#          правдоподобия аналитическим и численным методами
#----------------------------------------------------------
...
head(X)                                      # несколько элементов выборки
...
X_den <- dnorm(x    = X,                   # для каждой из реализаций выборки
               ...
               sd   = sigma)               # и стандартным отклонением sigma 
head(X_den)
X_den_log <- log(X_den)                    # считаем логарифмы наблюдения
q <- data.frame(                           
  "Observations"  = X,                     # наблюдение
  "Densities"     = X_den,                 # значение функции плотности
  "Log Densities" = X_den_log)             # логарифм функции плотности
head(q)
  ...
  value <- 1 / (sqrt(2 * pi) * sd) *      # осуществляем расчет по формуле
           exp(-(x - mean) ^ 2 /          # функции плотности нормального
                (2 * sd ^ 2))             # распределения с математическим
                                          # ожиданием mean и стандартным отклонением 
...
lnL <- function (par,                      # вектор параметров нормального распределения
...
{                                         
  mu     <- par[1]                         # для удобства создадим отдельные переменные под
  sigma2 <- par[2]                         # значения оптимизируемых параметров
  
  if(sigma2 <= 0)                          # проверяем, что оптимизатор
  {                                        # не использует отрицательные значения
    return(NA)                             # для дисперсии и в противном
  }                                        # случае возвращаем пропуск
  
  sigma <- sqrt(sigma2)                    # считаем стандартное отклонение
...
lnL(par = c(mu, sigma2), X = X)            # с использованием истинных значений
                                           # параметров распределения
lnL(par = c(10, 100),    X = X)            # в произвольной точке
lnL(par = c(5, 8 ^ 2),   X = X,            # в произвольной точке и вернем
...
mu_MLE_0     <- mean(X)
sigma2_MLE_0 <- var(X) * (n - 1) / n       # по умолчанию  функция var() считает 
                                           # скорретированное выборочную дисперсию,
                                           # а ММП оценка параметра 
                                           # sigma2 совпадает с нескорректированой
...
sigma2_MLE_1 <- opt_MLE$par[2]                      # реализация оценки sigma
...
df <- data.frame("Analytical" = c(mu_MLE_0, sigma2_MLE_0), 
                 "Numeric"    = c(mu_MLE_1, sigma2_MLE_1),
                 "Real"       = c(mu, sigma2))
...
#         начальную точку mu = 1 и sigma2 = 4.
...
#         подберите начальную точку  для mu и sigma2, после чего оцените
#         параметры mu и sigma2 с помощью метода максимального 
...
#               Подсказка: используйте пакет mnorm и входящие
#                          в него функции rmnorm() и dmnorm()
...
#----------------------------------------------------------
# Часть 2. Расчет реализаций оценок ковариационной матрицы 
#          оценок метода максимального правдоподобия с 
#          использованием различных подходов и построение 
...
cov_MLE_0 <- matrix(c(sigma2_MLE_0 / n, 0,            # осуществляем расчет
                      0, 2 * sigma2_MLE_0 ^ 2 / n), 
                    ncol = 2)
...
lnL_hessian <- hessian(func = lnL,                       # рассчитываем Гессиан
                       x    = c(mu_MLE_1, sigma2_MLE_1), # численным методом
...
boot_est  <- data.frame("mu_MLE"     = rep(NA, boot_iter), # каждую итерацию складываем оценки
                        "sigma2_MLE" = rep(NA,             # в соответствующие строки датафрейма
                                           boot_iter))   
head(boot_est)
for(i in 1:boot_iter)
{
  ...
  boot_est$sigma2_MLE[i] <- var(X_boot) * ((n - 1) / n) # можно было и численно через optim()
...
J <- jacobian(func = lnL,                             # рассчитаем Якобиан функции, возвращающей
              x    = c(mu_MLE_1, sigma2_MLE_1),       # логарифмы вкладов наблюдений в функцию
              X    = X,                               # правдоподобия
              is_aggregate = FALSE)            
...
# Сравним результаты полученных доверительных
...
c(q_L_4, q_R_4)                                       # больших реализаций оценок my
...
# Создадим функцию:
# g(mu, sigma2) = (sigma ^ 0.5) * exp(mu)
g <- function(x)          
{
  mu     <- x[1]                    # достаем параметры
  sigma2 <- x[2]                    # из вектора параметров
  sigma  <- sqrt(sigma2)
...
g_grad_est <- grad(func = g,                         # найдем градиент численно, но
                   x    = c(mu_MLE_1, sigma2_MLE_1)) # можно было бы и аналитически
...
# матрицы
g_MLE_est <- g(c(mu_MLE_1, sigma2_MLE_1))                # ММП оценка g
...
#     случае mu и sigma
# g(x) - вектор столбец, содержащий по строкам
...
# H0: Rx = 0, где R это матрица с числом столбцов,
...
{
  mu       <- theta                # учитываем ограничения, наложенные
  sigma2   <- 10 * theta + 6       # на параметры распределения
  sigma    <- sqrt(sigma2)
  L_vector <- dnorm(x    = X,  
                    mean = mu,                   
                    sd   = sigma)            
...
sigma2_MLE_R <- 10 * theta_MLE + 6                  # ММП оценка sigma при заданном ограничении
...
# Тест Вальда:
g         <- 10 * mu_MLE_1  + 6 - sigma2_MLE_1
g_grad    <- matrix(c(10, -1), ncol = 1)
W         <- t(g) %*% qr.solve(t(g_grad) %*%        # вычисляем тестовую статистику
                               cov_MLE_1 %*% 
                               g_grad) %*% g
...
lnL_fisher_R <- -hessian(func = lnL,         # оцениваем информацию Фишера
                          x   = est_R,       # как умноженный на минус
                          X   = X)           # единицу Гессиан
LM           <- t(lnL_grad_R) %*% 
                solve(lnL_fisher_R) %*% 
                lnL_grad_R
...
#               гипотезу о том, что его компоненты независимы и одинаково 
#               распределены
...
#         1*)    H0: b1 = 0
#         2*)    H0: b1 = b2
#         3*)    Предельный эффект x1 на E(y|x1, x2) 
#                при x1 = 2 и x2 = 3 равен 0
#         4*)    Предельный эффект x2 на E(y|x1, x2) 
#                при x1 = 2 и x2 = 3 равен 0
#         5**)   Предельный эффект x1 и x2 на E(y|x1, x2) 
#                при x1 = 2 и x2 = 3 равны 0
...
ncp <- 10                                  # параметр отклонения от центра
X   <- rt(n   = n,                         # симулируем выборку объема n 
                                           # из распределения стьюдента
          df  = df,                        # со степенями свободы df
          ncp = ncp)                       # и параметром отклонения от центра ncp
...
KLIC_opt <- optim(fn           = KLIC, 
                  par          = c(mean_QMLE * 0.7,  # возьмем достаточно
...
# ЗАДАНИЯ (* - непросто, ** - сложно, *** - брутально)
# 4.1*.   Повторите данный эксперимент используя логистическое
#         распределение вместо центрированного распределения Стьюдента
# 4.2**.  Повторите данный эксперимент используя логистическое
#         распределение с двумя параметрами вместо нормального распределения
# 4.3***. Повторите данный эксперимент с любыми двумя двумерными распределениями