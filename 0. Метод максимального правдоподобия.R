...
X_den <- dnorm(x = X,                      # для каждой из реализаций выборки
               ...
               mean = mu,                  # с математическим ожиданием mu
               sd = sigma)                 # и стандартным отклонением 
head(X_den)
X_den_log <- log(X_den)                    # считаем логарифмы
q <- data.frame(                           # наблюдения
  "Observations" = X,                      # функция плотности посчитанная по наблюдениям
  "Densities" = X_den,                     # и ее логарифм
  "Log Densities" = X_den_log)    
head(q)
...
for(i in 1:boot_iter)
{
  ...
  boot_est$sigma2_MLE[i] <- var(X_boot)  *            # можно было и численно через optim()
                            ((n - 1) / n) 
}
head(boot_est)
