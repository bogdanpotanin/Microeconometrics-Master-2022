...
library("switchSelection")                              # оценивание систем
library("GJRM")                                         # бинарных уравнений
...
model_bp <- gjrm(formula = list(default_formula,        # задаем лист, состоящий из 
                 ...
                 copula = "N")                          # задаем тип совместного распределения
...
# Альтернативный способ оценивания, не требующий
# ручных расчетов значимости
model_bp2 <- mvoprobit(formula = list(default_formula,
                                      stable_formula),
                       data = h)                            
summary(model_bp2) 

# p_value <- function(param.est, sd.error)
# {
#   z <- param.est / sd.error
#   p <- 2 * min(pnorm(z), 1 - pnorm(z))
#   return(p)
# }
# p_value(rho_est, std_rho)
...
p_1_2 <- predict(model_bp2, group = c(1, 1))
head(p_1_2, 5)
...
p_1_n2 <- pbivnorm(x = cbind(default_li, -stable_li),   # аргументы функции распределения
                   ...
                   rho = -rho_est)
head(p_1_n2, 5)
# альтернатинвый способ
p_1_n2 <- predict(model_bp2, group = c(1, 0))
...
head(p_1_cond_n2, 5)
# альтернативный способ
p_1_cond_2 <- predict(model_bp2, group = c(1, 1), 
                      given_ind = 2)
p_1_cond_n2 <- predict(model_bp2, group = c(1, 0), 
                       given_ind = 2)
...
model_bp3 <- gjrm(formula = list(default_formula,       # задаем лист, состоящий из 
                                 stable_formula),       # обеих формул
                  data = h,
                  model = "B",                          # указываем тип модели как систему
                                                        # бинанрных уравнений
                  ...
                  copula = "N")                           # задаем тип совместного распределения
                                                        # случайных ошибок (копулу)
summary(model_bp3)                                      # посмотрим на результат
rho_est <- model_bp3$theta

# Альтернативный способ оценивания
model_bp4 <- mvoprobit(formula = list(default_formula,
                                      stable_formula),
                       data = h)                            
summary(model_bp4) 
...
default_li <- predict(model_bp3, eq = 1, 
                      newdata = Boris)
stable_li <- predict(model_bp3, eq = 2, 
                     newdata = Boris)

# Оценим вероятности для Бориса
  # P(default = 1)
p_1 <- predict(model_bp3, eq = 1, type = "response",   # вероятность дефолта
               newdata = Boris)
  # P(stable = 1)
p_2 <- predict(model_bp3, eq = 2, type = "response",   # вероятность стабильной
...
# Альтернативный способ
  # P(default = 1)
p_1 <- predict(model_bp4, group = c(1, -1), newdata = Boris)
  # P(stable = 1)
p_2 <- predict(model_bp4, group = c(-1, 1), newdata = Boris)
  # P(default = 1, stable = 1)
p_1_2 <- predict(model_bp4, group = c(1, 1), newdata = Boris)

# Оценим вероятность того, что у Бориса будет
# дефолт при условии, что он у него стабильная работа
  # P(default = 1 | stable = 1)
p_1_cond_2 <- p_1_2 / p_2
p_1_cond_2 <- predict(model_bp4, group = c(1, 1), 
                      newdata = Boris, given_ind = 2)
...
Boris_new$stable <- 0                                 # представим, что у Бориса нет  
                                                      # стабильной работы
  # аналитический (сложный) способ
default_li_new <- predict(model_bp3, eq = 1,          # пересчитаем оценку линейного индекса
                          newdata = Boris_new)  
...
p_1_cond_n2 <- predict(model_bp4, group = c(1, 0), 
                       newdata = Boris_new, 
                       given_ind = 2)

# Оценим предельный эффект стабильной работы
# на вероятность дефолта для Бориса
 # P(default = 1 | stable = 1) - P(default = 1 | stable = 0)
ME_stable <- p_1_cond_2 - p_1_cond_n2

# Условный средний эффект воздействия
# CATE = P(default1 = 1 | Boris) - P(defaul0 = 1 | Boris)
p1 <- predict(model_bp4, group = c(1, -1), newdata = Boris)
p0 <- predict(model_bp4, group = c(1, -1), newdata = Boris_new)
CATE <- p1 - p0

# Средний эффект воздействия
# ATE = P(default1 = 1) - P(default1 = 0)
  # оценим P(default1 = 1) для всех наблюдений в выборке
h_stable1 <- h
h_stable1$stable <- 1
p1 <- predict(model_bp4, group = c(1, -1), newdata = h_stable1)
  # оценим P(default1 = 0) для всех наблюдений в выборке
h_stable0 <- h
h_stable0$stable <- 0
p0 <- predict(model_bp4, group = c(1, -1), newdata = h_stable0)
  # ATE
ATE <- mean(p1 - p0)
print(ATE)
...
# 3.3*.  Оцените эффект воздействия стабильной работы
...
model.1 <- mvoprobit(list(higher ~ age + I(age ^ 2 / 100) + shigher,
                          work ~ age + higher + nchild + health),
                     data = data)
summary(model.1)

# Учтем гетероскедастиность
model.2 <- mvoprobit(list(higher ~ age + I(age ^ 2 / 100) + shigher | age,
                          work ~ age + higher + 
                                 nchild + health | health),
                     data = data)
summary(model.2)

# Рассмотрим полупараметрическую модель
model.3 <- mvoprobit(list(higher ~ age + I(age ^ 2 / 100) + shigher | age,
                          work ~ age + higher + 
                            nchild + health | health),
...
model.4 <- mvoprobit(list(higher ~ age + I(age ^ 2 / 100) + shigher,
                          work ~ age + higher + nchild + health),
                     data = data, 
...
# d P(work = 1 | higher = 1) / d age
me.age <- predict(model.1, newdata = Alice, group = c(1, 1), given_ind = 1,
                  me = "age")

# Протестируем значимость этого предельного эффекта
me.age.test <- predict(model.1, newdata = Alice, group = c(1, 1), given_ind = 1,
                       me = "age", se = TRUE)
summary(me.age.test)

# Оценим средний эффект воздействия высшего образования на вероятность занятости
CATE <- predict(model.1, group = c(-1, 1), me = "higher", eps = c(0, 1))
ATE <- mean(CATE)

# Протестируем гипотезу о значимости среднего предельного эффекта
ATE_fn <- function(object)
{
  CATE <- predict(object, group = c(-1, 1), me = "higher", eps = c(0, 1))
  ATE <- mean(CATE)
}
# Используем базовую модель
test.1 <- delta_method(object = model.1, fn = ATE_fn)
summary(test.1)
# Применим полупараметрическую модель
test.3 <- delta_method(object = model.3, fn = ATE_fn)
summary(test.3)
