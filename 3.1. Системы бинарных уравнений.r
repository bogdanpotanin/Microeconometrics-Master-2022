...
head(p_1_cond_n2, 5)
# альтернативный способ
p_1_cond_2 <- predict(model_bp2, group = c(1, 1), given_ind = 2)
p_1_cond_n2 <- predict(model_bp2, group = c(1, 0), given_ind = 2)
...
pc <- predict(model.1, newdata = Alice, group = c(1, 1), given_ind = 2)
pc <- p.11 / p.1x

# Оценим предельный эффект возраста на условную вероятность
me.age <- predict(model.1, newdata = Alice, group = c(1, 1), given_ind = 2,
                  me = "age")

# Протестируем значимость этого предельного эффекта
me.age.test <- predict(model.1, newdata = Alice, group = c(1, 1), given_ind = 2,
                       me = "age", se = TRUE)
summary(me.age.test)

# Оценим средний эффект воздействия высшего образования на вероятность занятости
TE <- predict(model.1, group = c(-1, 1), me = "higher", eps = c(0, 1))
ATE <- mean(TE)

# Протестируем гипотезу о значимости среднего предельного эффекта
ATE_fn <- function(object)
{
  TE <- predict(object, group = c(-1, 1), me = "higher", eps = c(0, 1))
  ATE <- mean(TE)
}
