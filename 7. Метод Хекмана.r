...
# нормального распределения
sigma <- 5                                        # стандартное отклонение случайной
...
sigma(model_ls)                                      # оценка стандартного отклонения
  # метод Хекмана, основанный на ММП
model_mle <- msel(                              
...
data.frame("Real"          = rho,                    # истиннoе значение
           ...
           "Heckman MLE"   = as.numeric(rho_mle))
...
# Создадим индивида
Boris <- data.frame("age" = 0.1, "hobby" = 0.3, "income" = 0.5)
...
cat_prob <- predict(model_mle, 
  ...
  # оценим линейный предиктор без константы
...
lambda_est_1 <- dnorm(cat_li) / pnorm(cat_li)      # оценка обратного 
lambda_est_0 <- dnorm(cat_li) / pnorm(-cat_li)     # отношения Миллса
...
# на затраты на содержание кота на E(y*|z=1)
  # автоматически
me_hobby_1 <- predict(model_mle, 
                      group2 = 0,       group   = 1, 
                      type   = "val",   newdata = Boris,
                      me     = "hobby", test    = TRUE)
summary(me_hobby_1)
  # аналитически
...
me_hobby_3      <- (cost_cond_1_eps - cost_cond_1) / eps
  # сравнение
c(auto       = me_hobby_1$tbl$val, 
  analytical = me_hobby_2, 
  numeric    = me_hobby_3)
...
cat_li   <- predict(model_1, type = "link")             # оценим линейный предиктор
...
h$cost_b <- as.numeric(h$cost > 2)                        # бинарная переменная
                                                          # на превышающие некоторый
                                                          # объем затраты на кота
table(h$cost_b)

# Построим модель
model <- msel(formula = list(cat    ~ age + hobby,
                             cost_b ~ hobby + income),
              data = h)
summary(model)
...
probs2 <- predict(model, group = c(-1, 1), type = "prob", me = "hobby")
head(probs2)
...
model1 <- msel(formula  = work  ~ age + nchild + health + bachelor + master,
...
c(parametric = AIC(model1), semiparametric = AIC(model2))
...
model3 <- msel(formula   = work  ~ age + nchild + health + bachelor + master,
               formula2  = lwage ~ age + bachelor + master,
               estimator = "2step", marginal = list(logistic = NULL),         
               data      = data, degrees = 2)
summary(model3)

# Проверим гипотезу о том, что отдача от бакалаврского
# и магистерского уровней образования одинаковая
...
test <- test_msel(model1, fn = fn_test)
summary(test)

#---------------------------------------------------
# Часть 6. Метод Хекмана с инструментальными
#          переменными
#---------------------------------------------------

# Применим метод Хекмана с инструментальными переменными на уровни образования,
# включая их уравнения до уравнения основной переменной
model4 <- msel(formula  = work          ~ age + nchild    + health + 
                                                bachelor  + master,
               formula2 = list(bachelor ~ age + sbachelor + smaster,
                               master   ~ age + sbachelor + smaster,
                               lwage    ~ age + bachelor  + master),
               data     = data, estimator = "2step")
summary(model4)

# Сравним оценки отдачи от образования
coef_educ1 <- coef(model1, 
                   type = "coef2", eq2 = 1, regime = 0)[c("bachelor", "master")]
coef_educ4 <- coef(model4, 
                   type = "coef2", eq2 = 3, regime = 0)[c("bachelor", "master")]
cbind("No IV" = coef_educ1, "IV" = coef_educ4)
...
model_mle <- msel(formula = cat ~ age + hobby,                     
