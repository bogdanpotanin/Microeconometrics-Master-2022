...
default_formula <- default ~ I(log(income)) + age + I(age ^ 2 / 100) +
                               educ_2 + educ_3 + I(sqrt(credit)) + stable

# Оценим параметры двумерной пробит модели
model <- msel(formula = list(stable_formula, default_formula), data = h)                            
summary(model) 
...
p_d1_s0 <- predict(model, type = "prob", group = c(0, 1), newdata = Boris,
                   exogenous = list(stable = 0), given_ind = 1)
...
model.0 <- msel(list(work ~ age + higher + nchild + health),
                data = data)
summary(model.0)

# Учтем эндогенность образования, включив дополнительное уравнение
...
# Оценим средний эффект воздействия высшего образования на занятость среди
# тех, кто имеет высшее образование
ATET_fn <- function(object)
{
  treated <-  object$data[object$data$higher == 1, ]
  p1      <- predict(object, type = "prob", group = c(1, 1), given_ind = 1,
                     exogenous = list(higher = 1), newdata = treated)
  p0      <- predict(object, type = "prob", group = c(1, 1), given_ind = 1,
                     exogenous = list(higher = 0), newdata = treated)
  ATET    <- mean(p1 - p0)
  return(ATET)
}
ATET <- test_msel(model.1, ATET_fn)
summary(ATET)

# Альтернативная версия кода станет доступна
# после исправления бага в версии 2.0.1 пакета switchSelection
# ATET <- predict(model.1, type = "prob", group = c(1, 1), 
#                 me = "higher", eps = c(0, 1), test = mean,
#                 data = model.1$data[model.1$data$educ == 1, ],
#                 given_ind = 1)
# summary(ATET)

#---------------------------------------------------
# Часть 6. Система бинарного выбора с эндогенным
#          переключением
#---------------------------------------------------

# Сформируем отдельные переменные под потенциальные исходы
  # занятость индивида в состоянии, когда он имеет высшее образование
data$work1                          <- NA
data$work1[which(data$higher == 1)] <- data$work[which(data$higher == 1)]
  # занятость индивида в состоянии, когда он не имеет высшего образования
data$work0                          <- NA
data$work0[which(data$higher == 0)] <- data$work[which(data$higher == 0)]
  # посмотрим на результат
head(cbind(higher = data$higher, work0 = data$work0, work1 = data$work1))

# Оценим модель с эндогенным переключением
# Важно - нужно убрать переменную переключения из формулы уравнения
#         целевой переменной
model <- msel(list(higher ~ age + I(age ^ 2 / 100) + shigher,
                   work0  ~ age + nchild + health,
                   work1  ~ age + nchild + health),
              data = data)
summary(model)

# Оценим средний эффект воздействия
ATE_fn2 <- function(object)
{
  p_work1 <- predict(object, type = "prob", group = c(-1, -1,  1))
  p_work0 <- predict(object, type = "prob", group = c(-1,  1, -1))
  ATE     <- mean(p_work1 - p_work0)
}
ATE2 <- test_msel(model, fn = ATE_fn2)
summary(ATE2)

# Оценим средний эффект воздействия на подвергшихся воздействию
ATET_fn2 <- function(object)
{
  treated <- object$data[object$data$higher == 1, ]
  p_work1 <- predict(object, type = "prob", group = c(1, -1,  1),
                     given_ind = 1, newdata = treated)
  p_work0 <- predict(object, type = "prob", group = c(1, 1,  -1),
                     given_ind = 1, newdata = treated)
  ATET    <- mean(p_work1 - p_work0)
}
ATET2 <- test_msel(model, fn = ATET_fn2)
summary(ATET2)

# Сравним оценки иерархической пробит модели и модели
# с эндогенным переключением
data.frame("ATE" = rbind("Endogenous regressor" = ATE$tbl$val, 
                         "Endogenous switching" = ATE2$tbl$val),
           "ATET" = rbind(ATET$tbl$val, ATET2$tbl$val))