...
gamma_oprobit <- coef(model_oprobit, type = "coef", eq = 1)         # оценки порядкового пробита
...
probs <- cbind(Probit  = predict(model_probit, type = "response"),
               Ordered = as.numeric(predict(model_oprobit, type = "prob", 
                                            group = 2) + 
                                    predict(model_oprobit, type = "prob", 
                                            group = 3)))
head(probs)
