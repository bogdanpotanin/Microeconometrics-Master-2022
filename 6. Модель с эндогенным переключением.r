...
cov.2st <- coef(model.2st, type = "cov12")[[1]]            # оценки ковариаций между
cov.2st.0 <- cov.2st[1]                                    # случайными ошибками
cov.2st.1 <- cov.2st[2]                    
sigma.2st <- sigma(model.2st)                              # стандартное отклонение
sigma.2st.0 <- sigma.2st[1]                                # случайной ошибки                          
sigma.2st.1 <- sigma.2st[2]                                
rho.2st.0 <- cov.2st.0 / sigma.2st.0
rho.2st.1 <- cov.2st.1 / sigma.2st.1
...
data.frame("Real" = rho.1,                                 # истиннoе значение
           "Least Squares" = 0,                            # МНК оценки
           "Switch 2ST" = rho.2st.1,                       # оценки двухшаговые
...
data$educ <- 0
data$educ[data$bachelor == 1] <- 1
data$educ[data$master == 1] <- 2
model4 <- mvoprobit(formula = list(work ~ age + nchild + health + 
                                          bachelor + master,
...
data$slwage[data$swork == 0] <- Inf
model5 <- mvoprobit(formula = list(work ~ age + nchild + health + 
                                          bachelor + master,
